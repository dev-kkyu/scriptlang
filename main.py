...
window.geometry("1050x600+100+100")  # 창사이즈설정
...
Title = Label(frame1left, text="언제타 지하철", font = ("맑은 고딕",44, "bold"), height=1, width=13, padx = 10)
Title.pack(fill = "both")

empty = Label(frame1right, width = 1, bd=0)
empty.pack(side = "left", fill = "both")
...
frame2 = Frame(Main, relief="solid", bd=1)
...
for i in range(9):
    if i==0:
        emptyLabel.append(Label(frame2, width = 3, height = 5))
    else:
        emptyLabel.append(Label(frame2, width = 1))
    subwayLineButton.append(Button(frame2, text=str(i+1)+"호선", font = ("맑은 고딕", 15, "bold"), width = 6, height = 1, fg = "white", bg = "#92D050"))

    emptyLabel[i].pack(side = "left")
    subwayLineButton[i].pack(side = "left")
...
subwaylist = Label(frame3left, text = "호선별 역 목록", width = 71, height = 2, relief = "solid", bd=1, bg = "#64C044")
...
test1label = Label(frame3left, text = "left",  height = 16)
...
graphframe = Frame(frame4, relief = "solid", bd=1, width = 750)
